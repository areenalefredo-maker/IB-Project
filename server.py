...
# ════════════════════════════════════════════════════════════════════════
def _apply_qp(doc):
    # Cosmetic linguistic changes ONLY — no compounds, no values, no equations touched.

    # "A student" → "A candidate"
    p = doc[1]
    _sp(p, "A student determined the percentage",
        "A student", "A candidate")

    # "container" → "vessel"  (Q6, page 14 — pure noun swap, no science)
    p = doc[13]
    _sp(p, "placed in a 1.00"+TS, "container", "vessel")

def _apply_ms(doc):
    # No changes to Mark Scheme — original values preserved.
    pass
