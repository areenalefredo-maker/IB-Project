...
def modify_pdf(input_bytes, replacements):
    ...
    for page_num, old, new in replacements:
        ...
        for rect in page.search_for(old):
            # Get font info
            size, color, fontname = 11.0, (0,0,0), "helv"
                ...
                for line in b["lines"]:
                    for span in line["spans"]:
                        if fitz.Rect(span["bbox"]).intersects(rect):
                            ...
                            f = span["font"]
                            fontname = "hebo" if "Bold" in f else "hebi" if "Italic" in f else "helv"
                            break
            # White out + rewrite
            page.draw_rect(fitz.Rect(rect.x0-1,rect.y0-1,rect.x1+2,rect.y1+2),
                          color=(1,1,1), fill=(1,1,1), overlay=True)
            page.insert_text(fitz.Point(rect.x0, rect.y1-2), new,
                           fontname=fontname, fontsize=size, color=color, overlay=True)
        print(f"  p{page_num}: '{old[:30]}' → '{new[:30]}'")
