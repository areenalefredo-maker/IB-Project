...
def apply_swap_to_page(page, old_text, new_text):
    """Replace old_text with new_text on page. Searches all spans."""
    count = 0
    ...
    for b in page.get_text("dict")["blocks"]:
        ...
        for line in b["lines"]:
            for span in line["spans"]:
                st = span["text"]
                if old_stripped not in st:
                    continue

                new_st = st.replace(old_stripped, new_text.strip(), 1)
                ...
                ins  = fitz.Rect(sr.x0, sr.y0-1,
                                 min(sr.x0+nw+10, pr.x1-5), sr.y1+2)
...
def modify_pdf(input_bytes, pdf_type, api_key=""):
    ...
    if pdf_type == 'qp':
        ...
        for pg_num in range(len(doc)):
            ...
            print(f"  Page {pg_num+1}: Claude returned {len(swaps)} swaps: {swaps}")
            for swap in swaps:
                old_w = swap.get("old", "").strip()
                new_w = swap.get("new", "").strip()
                if old_w and new_w and old_w != new_w:
                    n = apply_swap_to_page(page, old_w, new_w)
                    print(f"    '{old_w[:30]}' → '{new_w[:30]}': {n} applied")
                    total_swaps += n
